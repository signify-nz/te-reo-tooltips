...
tinymce.PluginManager.add('TeReoPlugin', (editor, url) => {
  ...
  function checkForMatches(content, shortcode = true) {
    ...
    if (shortcode) {
      ...
      openTag =
        '<span class="TeReoTooltip" style="text-decoration: underline 1px dashed;">';
      closeTag = '</span>';
      openRegex = '(?<![a-zA-Z0-9])(';
    ...
    dictionaryMap.forEach((value, key) => {
      ...
      alteredContent = alteredContent.replace(
        regex,
        (match) => openTag + encode(match) + closeTag
      );
    });
  ...
  function treeWalk(rng) {
    ...
    do {
      ...
      if (
        ...
        const result =
          escapeHtml(currentNode.nodeValue.substr(0, rng.startOffset)) +
        ...
        if (currentNode.isEqualNode(rng.endContainer) && foundStartNode) {
          const result =
            checkForMatches(
              currentNode.nodeValue.substr(0, rng.endOffset),
              false
          ...
          const result =
            currentNode.nodeValue.substr(0, rng.startOffset) +
            checkForMatches(
              currentNode.nodeValue.substr(rng.startOffset),
              false
            );
          garbage = addHtmlToTextNode(currentNode, result);
          foundStartNode = true;
  ...
  function newWordPair(base, destination, destinationAlternate, id) {
    if (!base || !destination) {
      ...
      tinymce.activeEditor.windowManager.alert(
        `This word already has a translation! (${dictionaryMap.get(base)})`
      );
    } else {
      ...
      request.onreadystatechange = function handleUpdateResponse() {
        if (this.readyState === 4 && this.status === 200) {
          ...
          if (destinationAlternate) {
            tinymce.activeEditor.windowManager.alert(
              `Successfully added wordpair '${base}', '${destination}', '${destinationAlternate}'.`
            );
          } else {
            tinymce.activeEditor.windowManager.alert(
              `Successfully added wordpair '${base}', '${destination}'.`
            );
          }
        }
      ...
      (match) => {
        ...
        dictionaryMap.forEach((value, key) => {
          if (
            ...
            restoration = `[TT]${match.slice(
              startOffset,
              match.length - endOffset
            )}[/TT]`;
          }
        });
        if (!restoration) {
    ...
    editor.windowManager.open({
      ...
      body: {
        ...
        items: [{
          ...
          label: 'Base',
          id: 'BaseInputTextBox',
          tooltip: 'An untranslated word/phrase',
        },
        {
          type: 'input',
          name: 'Destination',
          size: 40,
          label: 'Translation',
          id: 'DestinationInputTextBox',
          tooltip: 'A translated word/phrase',
        },
        {
          type: 'input',
          ...
          id: 'DestinationAlternateInputTextBox',
          tooltip: 'An alternate translated word/phrase',
        },
        {
          type: 'listbox',
          name: 'Dictionary',
      ...
      onSubmit: (api) => {
        ...
        const selectedID = parseInt(data.Dictionary, 10);
        const base = data.Base.replace(/[\u200B-\u200D\uFEFF]/g, '');
        const dest = data.Destination;
        const alternateDest = data.DestinationAlternate;
        newWordPair(
          base,
          dest,
          alternateDest,
  ...
  editor.ui.registry.addMenuItem('addToDictionary', {
  ...
  editor.ui.registry.addMenuItem('contextTranslate', {
    text: 'Translate',
    onAction: () => {
      bookmark = tinymce.activeEditor.selection.getBookmark(2, true);
