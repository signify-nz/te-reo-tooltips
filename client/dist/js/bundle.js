...
tinymce.PluginManager.add('TeReoPlugin', function (editor, url) {
  ...
  function checkForMatches(content) {
    ...
    dictionaryMap.forEach(function (value, key) {
      ...
      alteredContent = alteredContent.replace(regex, function (match) {
        return openTag + encode(match) + closeTag;
      });
    });
  ...
  function getSubsiteId() {
    var subsiteSelect = document.getElementById('SubsitesSelect');
    if (subsiteSelect) {
      var subsiteId = subsiteSelect.value;
      return subsiteId;
    } else {
      return 0;
    }
  ...
  function translateThroughAPI(textContent) {
    var request = new XMLHttpRequest();
    var path = '/api/v1/dictionary/translateThroughInterface?SubsiteID=' + getSubsiteId();
  ...
  function getDictionary(id) {
    var request = new XMLHttpRequest();
    var path = '/api/v1/dictionary/dictionaries/' + id + '?SubsiteID=' + getSubsiteId();
  ...
  function getDictionaries() {
    var request = new XMLHttpRequest();
    var path = '/api/v1/dictionary/index/?SubsiteID=' + getSubsiteId();
  ...
  function addToDictMenu(selection) {
    editor.windowManager.open({
      ...
      onSubmit: function onSubmit(api) {
        ...
        var selectedID = parseInt(data.Dictionary, 10);
        var base = data.Base.replace(/[\u200B-\u200D\uFEFF]/g, '');
        var dest = data.Destination;
        var alternateDest = data.DestinationAlternate;
        newWordPair(base, dest, alternateDest, selectedID);
      }
    });
  }
